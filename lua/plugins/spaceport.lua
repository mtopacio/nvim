return {
    ...
    opts = {
        ...
        -- This prevents the same directory from being repeated multiple times in the
        -- recents section. For example, I have replaceDirs set to
        -- { {"~/projects", "_" } } so that ~/projects is not repeated a ton. Note every
        -- element is applied to the directory in order, so if you have { {"~/projects",
        -- "_"} } and you also want to replace ~/projects/foo with @, then you would
        -- need { {"~/projects/foo", "@"}, {"~/projects", "_"} }
        -- or { {"~/projects", "_"}, {"_/foo", "@"} }
        replaceDirs = {},
        ...
        -- What to do when entering a directory, personally I use "Oil .", but Ex is
        -- preinstalled with neovim
        projectEntry = "Ex",

        -- The farthest back in time that directories should be shown
        -- I personally use "yesterday" so that there aren't millions of directories on
        -- the screen. The possible values are: "pin", "today", "yesterday", "pastWeek", "pastMonth", and "later"
        lastViewTime = "later",
        ...
        --  For example, the following can be used to set different icons `{ file = " "
        --  , dir = " ", remaps = " ", pinned = " ", today = " ", yesterday = " ",
        --  week = " ", month = " ", long = " ", news = "󱀄 " }`
        icons = true,
